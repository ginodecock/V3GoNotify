...
#elif F_CPU >= 4000000L
#define BAUD_RATE   38400L   // 19200 also supported, but with significant error
#elif F_CPU >= 1000000L
...
int main(void) {
  ...
  DDRC |= _BV(PC2);		//ESP reset
  DDRB |= _BV(PB2);		//Esp power
  DDRC |= _BV(PC3);		//Esp IO0*/
  
  if (PIND & (1<<PD3)){		//Esp enter bootloader mode
	  ...
	  PORTB |= _BV(PB5);
	  PORTB |= _BV(PB2); //Esp power off
	  PORTC &= ~(_BV(PC3)); //Esp low IO0
	  //watchdogReset();
		asm volatile (
		"    ldi  r18, 21"	"\n"
		"    ldi  r19, 75"	"\n"
	  ...
	  PORTC |= _BV(PC2); //Esp High reset 
	do{
		if (PIND & (1<<PD0))		//RX0 ==> TX1
			PORTB |= _BV(PB3);
		else
			PORTB &= ~(_BV(PB3));
	  
		if (PINB & (1<<PB4))		//RX1 ==> TX0
			PORTD |= _BV(PD1);
		else
			PORTD &= ~(_BV(PD1));
	}while (1);
