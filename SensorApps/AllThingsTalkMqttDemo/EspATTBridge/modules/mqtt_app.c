...
uint32_t MQTTAPP_Setup(PACKET_CMD *cmd)
{
	REQUEST req;
	MQTT_Client *client;
...
uint32_t MQTTAPP_Lwt(PACKET_CMD *cmd)
{
	REQUEST req;
	MQTT_Client *client;
	CMD_Request(&req, cmd);
...
uint32_t MQTTAPP_Connect(PACKET_CMD *cmd)
{
	MQTT_Client *client;
	REQUEST req;
	uint16_t len;
...
uint32_t MQTTAPP_Disconnect(PACKET_CMD *cmd)
{
	MQTT_Client *client;
	REQUEST req;
	uint16_t len;
...
uint32_t MQTTAPP_Publish(PACKET_CMD *cmd)
{
	os_printf("CMD: Publish\r\n");
	MQTT_Client *client;
	REQUEST req;
	uint16_t len;
	uint8_t *topic, *data;
	...
	system_os_post(2, 0, (os_param_t)client);*/
	os_free(topic);
	os_free(data);
	return 1;

}
uint32_t MQTTAPP_Subscribe(PACKET_CMD *cmd)
{
	MQTT_Client *client;
	REQUEST req;
	uint16_t len;
