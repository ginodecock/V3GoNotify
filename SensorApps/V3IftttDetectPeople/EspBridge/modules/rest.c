...
uint32_t ICACHE_FLASH_ATTR REST_Setup(PACKET_CMD *cmd)
{
	...
	client->data = (uint8_t*)os_zalloc(512);
	...
	client->user_agent = (uint8_t*)os_zalloc(13);
	os_sprintf(client->user_agent, "GoNotify@g-dc");
	client->user_agent[12] = 0;
...
char *replace_str(char *str, char *orig, char *rep)
{
  ...
  char *buffer = (char*)os_zalloc(os_strlen(str)+ os_strlen(rep));
  char *p;

  if(!(p = (char*)os_strstr(str, orig)))  // Is 'orig' even in 'str'?
    return str;

  //strncpy(buffer, str, p-str); // Copy characters from 'str' start to 'orig' st$
  os_memcpy(buffer,str, p-str);
  buffer[p-str] = '\0';
  //buffer[p-str] = 0;
  os_sprintf(buffer+(p-str), "%s%s", rep, p+os_strlen(orig));
  //os_free(buffer);
  return buffer;
}
...
uint32_t ICACHE_FLASH_ATTR REST_Request(PACKET_CMD *cmd)
{
	...
	if(CMD_GetArgc(&req) == 3){
		...
		len = CMD_ArgLen(&req);
		body = (uint8_t*)os_zalloc(255);
		CMD_PopArgs(&req, body);
		body[len] = 0;
		os_printf("Heap1: %ld\n", (unsigned long)system_get_free_heap_size());
		if((q = (char*)os_strstr(body, "#IP#"))){  // Is 'orig' even in 'str'?
			os_printf("Heap2: %ld\n", (unsigned long)system_get_free_heap_size());
			struct ip_info info;
			wifi_get_ip_info(0, &info);
			insertString = (uint8_t*)os_zalloc(32);
			os_sprintf(insertString,"%d.%d.%d.%d",IP2STR(&info.ip));
			//os_sprintf(insertString,"%d.%d.%d.%d",(info.ip.addr>>0)&0xff, (info.ip.addr>>8)&0xff, (info.ip.addr>>16)&0xff, (info.ip.addr>>24)&0xff);
			insertString[os_strlen(insertString)] = 0;
			INFO("readip: %s",insertString);
			//static char buffer[128];		
			realLen = realLen + os_strlen(insertString) - 4;
			body = (uint8_t*)replace_str(body, "#IP#", insertString);
			os_printf("Heap3: %ld\n", (unsigned long)system_get_free_heap_size());
			}
		if((p = (char*)os_strstr(body, "#CHIPID#"))){  // Is 'orig' even in 'str'?
			//os_printf("Heap: %ld\n", (unsigned long)system_get_free_heap_size());
			os_printf("HeapTitle: %ld\n", (unsigned long)system_get_free_heap_size());
			char chipId[16];
			os_sprintf(chipId, "%s_%x", "GoNotify",system_get_chip_id());
			realLen = realLen + os_strlen(chipId) - 8;
			body = (uint8_t*)replace_str(body, "#CHIPID#", chipId);
			os_printf("HeapTitle: %ld\n", (unsigned long)system_get_free_heap_size());
			}
	}
    INFO("%s",body);
