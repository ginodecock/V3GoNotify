...
static void CMD_Task(os_event_t *events);
uint32_t CMD_Reset(PACKET_CMD *cmd);
...
LOCAL uint32_t CMD_Exec(const CMD_LIST *scp, PACKET_CMD *packet)
{
	uint32_t ret;
	uint16_t crc = 0;
